...
extern struct bflb_device_s *i2c0;  //32,33
extern struct bflb_device_s *i2c1;	//0,1
...
void oled_writea(unsigned char reg_addr, unsigned char *buff, char buf_size)
{
...
void OLED_Init(void)
{
	// OLED_GPIO_Init();			//先调用底层的端口初始化 in main
	...
	OLED_WriteCommand(0x4F);	//0x00~0xFF		亮度
